...
class FluidPipe(properties: Properties): BaseEntityBlock(properties), Linkable {
    override fun newBlockEntity(blockPos: BlockPos, blockState: BlockState): BlockEntity? {
        return FluidPipeBE(blockPos, blockState)
    }

    override fun unlink(level: ServerLevel, pos: BlockPos): Boolean {
        val be = level.getBlockEntity(pos) ?: return false
        if (be !is FluidPipeBE) return false
        if (be.id == -1) return false

        FluidPipeFluidTankHandler.removeTank(be.id)
    ...
    override fun link(level: ServerLevel, pos1: BlockPos, pos2: BlockPos): Boolean {
        ...
        if (be1 !is FluidPipeBE || be2 !is FluidPipeBE) {return false}
        ...
        val id = FluidPipeFluidTankHandler.createTank {
            val be1 = level.getBlockEntity(pos1)
            if (be1 is FluidPipeBE) { be1.setChanged() }

            val be2 = level.getBlockEntity(pos2)
            if (be2 is FluidPipeBE) { be2.setChanged() }

            (be1 is FluidPipeBE) && (be2 is FluidPipeBE)
        }
